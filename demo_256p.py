...
def compute_error(real,fake,label):
    return tf.reduce_mean(label*tf.expand_dims(tf.reduce_mean(tf.abs(fake-real),reduction_indices=[3]),-1),reduction_indices=[1,2])
    ...
    for epoch in range(1,201):
        ...
        for ind in np.random.permutation(2975)+1:
            ...
            if input_images[ind] is None:
                ...
                input_images[ind]=np.expand_dims(np.float32(scipy.misc.imread("data/cityscapes/RGB256Full/%08d.png"%ind)),axis=0)#train image
